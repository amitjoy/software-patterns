//package edu.tum.cs.lecture1.pse2014;
...
public class StructuralTest {
	...
	public void testHierarchy() throws ClassNotFoundException {
		...
		try {
			...
			assertEquals("Does not extend class " + superClazz + ".", superClazz, clazz.getSuperclass().getSimpleName());
		...
		try {
			JSONArray expectedInterfaces = expectedClazz.getJSONArray("interfaces");
			...
			for (int i = 0; i < expectedInterfaces.length(); i++) {
				...
				for (Class<?> actualInterface : actualInterfaces) {
					...
					assertEquals("Does not implement interface " + expectedInterface + ".", true, implementsInterface);
				}
			}
		} catch (JSONException e) {
			// The parameter is optional, so this is ok just don't do
	...
	@Test
	public void testFields() throws ClassNotFoundException, NoSuchFieldException {
		Class<?> clazz = Class.forName(StructuralTest.class.getPackage()
				.getName() + "." + clazzName);
		try {
	...
	@Test
	public void testMethods() throws ClassNotFoundException, NoSuchMethodException {
		Class<?> clazz = Class.forName(StructuralTest.class.getPackage()
				.getName() + "." + clazzName);
		try {
	...
	private void checkConstructors(Class<?> clazz,
		...
		for (int j = 0; j < expectedConstructors.length(); j++) {
			...
			for (Constructor<?> constructor : constructors) {
				...
				checkModifiers(constructor.getModifiers(),
						expectedModifiers);
			...
			if (!constructorExists) {
				assertEquals("Expected a constructor with the parameters " + expectedParameters + ".", true, constructorExists);
			}
		}
	}
	...
	private void checkFields(Class<?> clazz, JSONObject expectedFields)
		...
		while (expectedFieldsIterator.hasNext()) {
			...
			String expectedFieldType = expectedFields.getString(expectedFieldName);
	...
	private void checkType(String expectedType, Field actualField) {
		if (expectedType.contains("<")) {
			...
			if (genericType instanceof ParameterizedType) {
				Type actualType = ((ParameterizedType) actualField.getGenericType())
	...
	// FIXME: Overloaded methods not supported: Currently this method just picks the first mehtod, that has the proper name. This might lead to a test, that does not find a method even though it is present.
	private void checkMethods(Class<?> clazz, JSONObject jsonMethods)
		...
		while (methodsIterator.hasNext()) {
			...
			try {
				...
				// The parameter is optional, so this is ok just don't do anything
			}
			...
			if (actualMethod != null) {
				...
				for (int j = 0; j < parameterTypes.length; j++) {
					...
					assertEquals("Wrong parameters in method " + methodName + ".", parameterType,
							parameterTypes[j].getSimpleName());
				}
				String returnType = expectedMethod.getString("returnType");
				assertEquals("Wrong return type in method " + methodName + ".",returnType, actualMethod.getReturnType()
						.getSimpleName());
			} else {
				throw new NoSuchMethodException(methodName + " in class "
						+ clazz.getSimpleName());
			}
		}
	}
	...
	private void checkModifiers(int actualModifiers, JSONArray expectedModifiers) {
		for (int i = 0; i < expectedModifiers.length(); i++) {
			...
			String modifiersString = Modifier.toString(actualModifiers);
			...
			assertEquals(true, modifierIsSet);
		}
	}
