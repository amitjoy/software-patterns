...
public class Client {
	...
	public static void main(String[] args) {
		/**
		 * Guice Bootstrapping code method local classes for demonstration only.
		 */
		...
		class FlightEngineModule extends AbstractModule {
			@Override
			public void configure() {
				bind(FlightSupplier.class).annotatedWith(CSV.class).to(
						CSVSupplier.class);
				bind(FlightSupplier.class).annotatedWith(
						Names.named("xmlSupplier")).toInstance(
						new XMLSupplier());
		...
		class FlightUtilityModule extends AbstractModule {
			@Override
			protected void configure() {
				bindConstant().annotatedWith(Names.named("dateFormat")).to(
						"dd-MM-yy");
		...
		Injector injector = Guice.createInjector(new CSVSupplierModule(),
				new FlightEngineModule(), new ClientModule(),
	...
	public void makeRequest() {
		...
		List<SearchResponse> responseList = flightEngine
