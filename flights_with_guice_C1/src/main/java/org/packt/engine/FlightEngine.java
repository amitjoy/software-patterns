	...
	public List<SearchResponse> processRequest(SearchRequest flightSearchRQ) {
		...
		for (SearchResponse flightSearchRS : flightSupplier.getResults()) {
			if (flightSearchRS.getArrivalLocation().equals(
					...
					|| flightSearchRS.getDepartureLocation().equals(
							flightSearchRQ.getDeparture_location()))
				criteriaMatch = true;
			...
			if (flightSearchRS.getArrivalLocation().equals(
					...
					&& flightSearchRS.getDepartureLocation().equals(
							flightSearchRQ.getDeparture_location())
					&& (flightSearchRS.getValidDate().compareTo(
							flightSearchRQ.getFlightDate()) == 0)) {
				responseList.add(flightSearchRS);
			}
		}

		if (!criteriaMatch)
			throw new NoCriteriaMatchException(
					"Depart/Arrival Codes don't match our records");
		if (responseList.size() == 0)
			throw new NoFlightAvailableException(
					"No flights found for given specified date");
